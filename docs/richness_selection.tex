...
\label{eq:rho_prj}
\end{equation}
and \(\rhoprj\equiv0\) for \(\lambda\ge\lob\)
--- the \textsc{redMaPPer} ranking cut: only projected halos of
\emph{lower} richness than the target contaminate the target's
aperture. Rather than carry an indicator function through every
equation below, this cut is stated once, here, and from now on
enforced silently by writing every \(\lambda\)-integral with
upper limit \(\lob\) (never \(\infty\)). Here \(\lambda\) is the
projected halo's true richness; \(z\) is its redshift; \(\theta\)
is its angular separation from the target; \(w_z(z,\zob)\) is the
parabolic photo-\(z\) kernel quantifying the probability that a
galaxy at redshift \(z\) is counted in the target's aperture at
\(\zob\); \(f_A(\theta,\lob,\zob,\lambda,z)\in[0,1]\) is the
closed-form overlap fraction of the two \textsc{redMaPPer} disks
(of the target and the projected halo); and the factor \(\lambda\)
converts ``this halo is present'' into ``this halo contributes
\(\lambda\) counts''.

\citealt{Costanzi2026} (\texttt{papers/Costanzi2026.tex},
\S~``Projection kernel'', Fig.~1) states the same weight per
projected halo \(i\) as
\(\rhoprj_i = \lambda_i\,w(z_i,\zob)\,f_{A,i}\): both
\(w(z_i,\zob)\) --- the photo-\(z\) kernel above, which sets how
much line-of-sight weight the far zone carries and is \emph{not}
negligible --- and the geometric factor \(f_{A,i}\) are needed;
this subsection derives the latter.

The overlap fraction \(f_A\) encodes the \textsc{redMaPPer}
membership geometry: the model treats the projected halo's
\(\lambda\) member galaxies as uniformly distributed over the
halo's own richness disk, of angular radius
\(\theta_\lambda(\lambda,z) = R_\lambda(\lambda)(1+z)/\chi(z)\);
the target gains only those galaxies that fall inside \emph{its}
disk, of radius
\(\theta_{\lambda,{\rm ob}} \equiv \theta_\lambda(\lob,\zob)\).
For a centre separation \(\theta\) the expected gained count is
therefore \(\lambda\) times the fraction of the projector's disk
area that overlaps the target's:
\begin{equation}
f_A(\theta,\lob,\zob,\lambda,z)
\;=\;
\frac{A_{\rm ov}(\theta,\theta_{\lambda,{\rm ob}},\theta_\lambda(\lambda,z))}
     {\pi\,\theta_\lambda^2(\lambda,z)},
\label{eq:fA_def}
\end{equation}
with \(A_{\rm ov}\) the circle--circle intersection (``lens'')
area of the target and projector disks at centre distance
\(\theta\),
\begin{align}
A_{\rm ov}(\theta,\theta_{\lambda,{\rm ob}},\theta_\lambda)
=\;& \theta_{\lambda,{\rm ob}}^2
\arccos\!\Big(\frac{\theta^2+\theta_{\lambda,{\rm ob}}^2
                     -\theta_\lambda^2}{2\theta\,\theta_{\lambda,{\rm ob}}}\Big)
+ \theta_\lambda^2
\arccos\!\Big(\frac{\theta^2+\theta_\lambda^2
                     -\theta_{\lambda,{\rm ob}}^2}{2\theta\,\theta_\lambda}\Big)
\notag\\
&- \tfrac12\sqrt{\big[(\theta_{\lambda,{\rm ob}}+\theta_\lambda)^2-\theta^2\big]
                 \big[\theta^2-(\theta_{\lambda,{\rm ob}}-\theta_\lambda)^2\big]}
\label{eq:A_ov}
\end{align}
for \(|\theta_{\lambda,{\rm ob}}-\theta_\lambda|<\theta
<\theta_{\lambda,{\rm ob}}+\theta_\lambda\), with
\(A_{\rm ov}=\pi\min(\theta_{\lambda,{\rm ob}},\theta_\lambda)^2\)
for \(\theta\le|\theta_{\lambda,{\rm ob}}-\theta_\lambda|\)
(full containment) and \(A_{\rm ov}=0\) for
\(\theta\ge\theta_{\lambda,{\rm ob}}+\theta_\lambda\) (no contact).
Note the normalisation: the denominator of eq.~\eqref{eq:fA_def}
is the \emph{projector's} own disk area, not the target's, because
\(f_A\) is the fraction of the projector's galaxies captured by
the target aperture. This makes \(f_A\in[0,1]\) exact, with
\(f_A=1\) when the projector disk sits entirely inside the
target's (\(\theta \le \theta_{\lambda,{\rm ob}}-\theta_\lambda\),
always reachable at \(z\simeq\zob\) since \(\lambda<\lob\) implies
\(\theta_\lambda<\theta_{\lambda,{\rm ob}}\)) and \(f_A=0\) beyond
\(\theta_{\lambda,{\rm ob}}+\theta_\lambda\) --- so the
\(\theta\)-integral support is finite. \(A_{\rm ov}\) is
homogeneous of degree two in its three angular arguments, hence
\(f_A\) depends only on the ratio
\(\theta_\lambda/\theta_{\lambda,{\rm ob}}\) --- the property the
frozen-physics companion note
(\texttt{docs/richness\_selection\_frozen.tex}, \S4) exploits.
Implementation:
\texttt{richness\_selection.geometry.area\_overlap}, which always
normalises by the \emph{second} disk's area regardless of which
disk is larger; the appendix-A formula of \citealt{Costanzi2026}
instead normalises by
\(\pi\min(\theta_{\lambda,{\rm ob}},\theta_\lambda)^2\), so the two
disagree when a low-\(z\) foreground halo's own disk exceeds the
target's (\(\theta_\lambda>\theta_{\lambda,{\rm ob}}\), possible
even for \(\lambda<\lob\) since angular size grows as \(z\to0\)).
...
Because eq.~\eqref{eq:b_sigmoid} is linear in the plateaus
\((\bsmall,\blarge)\) and the sigmoid \(\sigma(\theta)\) carries no
\(\ltr\) dependence, the \(\ltr\)-marginalisation commutes with the
sigmoid assembly. Defining the \(\ltr\)-averaged plateaus
\begin{equation}
\bar{b}_{\rm small}(\lob,\zob)
\;=\; \int\!d\ltr\;\bsmall(\lob,\ltr,\zob)\,P(\ltr\mid\lob,\zob),
\qquad
\bar{b}_{\rm large}(\lob,\zob)
\;=\; \int\!d\ltr\;\blarge(\lob,\ltr,\zob)\,P(\ltr\mid\lob,\zob),
\label{eq:b_bar_plateaus}
\end{equation}
eq.~\eqref{eq:b_marg_lt} reduces to
\begin{equation}
\boxed{\;
\bsel(\lob,\zob,\theta)
\;=\; \bar{b}_{\rm small}(\lob,\zob)\,[\,1-\sigma(\theta)\,]
   + \bar{b}_{\rm large}(\lob,\zob)\,\sigma(\theta).
\;}
\label{eq:b_marg_sigmoid}
\end{equation}
Eq.~\eqref{eq:b_marg_sigmoid} is numerically identical to
eq.~\eqref{eq:b_marg_lt} (it is an algebraic rearrangement, not an
approximation), but shifts the \(\ltr\)-quadrature from \emph{inside}
the \(\theta\)-loop to a one-shot pre-computation at fixed
\((\lob,\zob)\). Downstream consumers that assemble \(\bsel(\theta)\)
at many \(\theta\)-nodes --- notably
\(\avg{\Sprj}\) and \(\avg{\DSprj}\) --- therefore only pay the cost
of the analytic sigmoid per call.

The \(\bsmall\)-denominator guard in eq.~\eqref{eq:b_small}
(\(|I_2-I_1|\to 0\) \(\Rightarrow\) \(\bsmall\!\leftarrow\!\blarge\))
is applied per-\(\ltr\) before the average, so the definition of
\(\bar{b}_{\rm small}\) in eq.~\eqref{eq:b_bar_plateaus} is
well-defined even when the denominator is small for a subset of the
\(\ltr\) grid.
...
\texttt{bias\_from\_precomp}; the sigmoid assembly at fixed
\(\ltr\) is \texttt{b\_lob\_theta}; and the
\(\ltr\)-marginalised plateaus
\((\bar{b}_{\rm small},\bar{b}_{\rm large})\) of
eq.~\eqref{eq:b_bar_plateaus} are cached by
\texttt{\_marginalised\_plateaus}. The public entry point
\texttt{b\_sel\_marginalised(theta, lob, zob)} evaluates
eq.~\eqref{eq:b_marg_sigmoid} directly from those cached plateaus.
...
\caption{Two-halo \(\avg{\Sprj(R\,|\,\lob,\ltr,\zob)}\) at
\((\lob,\zob)=(20,0.5)\), varying the projection contamination
\(\dprj\equiv\lob-\ltr\) (same colour palette as Fig.~1). \(b_{\rm sel}\)
is evaluated at fixed \(\ltr\) (not \(\ltr\)-marginalised) to match the
Fig.~1 exercise. Dotted line: \(R_{\rm excl}=R_\lambda(\lob)(1+\zob)\),
separating the miscentered-NFW-dominated inner regime from the
correlated \(\bsel\!\cdot\!\xinl\) large-scale regime. \(\dprj=0\)
sits anomalously low at small \(R\) because both \(b_{\rm sel}\) plateaus
collapse to the halo-bias aggregate in that limit.}
\label{fig:pedag_sigmaprj}
\end{figure}

\subsection{Lensing excess \(\avg{\DSprj(R\mid\lob,\zob)}\)}
\label{sec:DeltaSprj}

The two-halo observable returned by \texttt{DeltaSigmaPrj} is the
radial excess of \eqref{eq:Sprj},
\begin{equation}
\avg{\DSprj(R\mid\lob,\zob)} \;\equiv\;
\bar\Sigma^{\rm prj}(<\!R\mid\lob,\zob) \;-\;
\avg{\Sprj(R\mid\lob,\zob)}.
\label{eq:DSprj_def}
\end{equation}
Because \(R \to \bar{\Sigma}(<\!R)-\Sigma(R)\) is a linear functional
in the radial argument only, it commutes with the
\((\theta,z,M)\) integrals in eq.~\eqref{eq:Sprj}: the pipeline of
\S\ref{sec:theta_split}--\ref{sec:zgrid} carries over unchanged, and
the only substitution is at the NFW lookup,
\(\Smis\!\to\!\DSmis\equiv\bar\Sigma_{\rm mis}(<\!R)-\Smis(R)\), evaluated via
the companion \texttt{log\_deltasigma\_single} table shipped with
the offset-NFW kit. The full derivation, the scale argument for
adaptive \(\theta_{\max}\), and the factor-of-2 convention
cross-check against the analytical Wright \& Brainerd 2000 centred
NFW \(\Delta\Sigma\) live in \texttt{docs/delta\_sigma\_prj\_derivation.tex}.

\paragraph{Profile vs projection contamination.}
Figure~\ref{fig:pedag_delta_sigma} shows
\(\avg{\DSprj(R\mid\lob,\ltr,\zob)}\) at \((\lob,\zob)=(20,0.5)\),
varying \(\dprj=\lob-\ltr\) --- the same sweep as Fig.~1
(\(\bsel\)-sigmoid) and Fig.~\ref{fig:pedag_sigmaprj}
(\(\avg{\Sprj}\)). The shape is the familiar two-halo lensing bump
peaking around \(\sim R_\lambda\) (dotted \(R_{\rm excl}\) marker);
amplitude scales with \(\dprj\) because both \(b_{\rm sel}\)
plateaus (\(\bsmall,\blarge\)) grow with projection contamination
through Steps~4--5 of the bias pipeline.

\begin{figure}[!htbp]
\centering
\includegraphics[width=0.92\textwidth]{figs/pedag_delta_sigma_prj.png}
\caption{\(\avg{\DSprj(R\,|\,\lob,\ltr,\zob)}\) at
\((\lob,\zob)=(20,0.5)\), varying \(\dprj=\lob-\ltr\)
(\texttt{cl+LSS} correlation-excess piece, the class default).
Dotted line: \(R_{\rm excl}=R_\lambda(\lob)(1+\zob)\). Inner
cut-off from the LoS-slab exclusion; outer decay set by the joint
support of \(\DSmis(R\mid\Rmis\!\sim\!R)\) and \(\xinl(r)\).}
\label{fig:pedag_delta_sigma}
\end{figure}

\paragraph{Precision vs \texttt{scipy.quad}.}
Figure~\ref{fig:dsprj_quad} compares the production
\texttt{DeltaSigmaPrj} (\(n_{\rm per\_seg}\!=\!30\),
\(N_M\!=\!24\), \(N_z\!=\!80\), \(R_{\max}=30\,\hMpc\)) against a
nested-\texttt{scipy.quad} reference built by
\texttt{validations/delta\_sigma\_prj\_diagnostics.py} at the same
\((\lob,\zob)=(20,0.5)\) and \(R\in\{0.3,1,3,10\}\,\hMpc\). The
reference is parallelised across the 8 \((R,\text{kind})\) jobs via
\texttt{multiprocessing.Pool} (8-core wall clock \(\sim3\,\)min vs
\(\sim35\,\)min serial); quad tolerances
\(\epsilon_\theta\!=\!3\!\times\!10^{-3}\),
\(\epsilon_z\!=\!10^{-2}\). The residual panel shows
sub-\(0.5\%\) agreement on \(R\!\ge\!1\,\hMpc\) on both
\texttt{total} and \texttt{cl}. At \(R\!=\!0.3\) the residual reaches
\(\sim 2\%\): the \([1+b\,\bsel\,\xinl]\) bracket changes amplitude
sharply where \(\theta_{\rm excl}(z)\) and
\(\theta_{R}\!=\!R/D_A(\zob)\) coincide, and \texttt{scipy.quad}'s
adaptive refinement issues an \texttt{IntegrationWarning} at that
crossing. Tightening the quad tolerances by \(3\!\times\) shifts the
reference by \(<\!0.01\%\) (so the quad itself is converged), and
the GL-on-segments production side is self-converged to
\(<\!0.5\%\) between \(n_{\rm per\_seg}\!=\!30\) and \(120\): the
residual at \(R\!=\!0.3\) is therefore a real
quadrature-method difference at the smallest \(R\), not a bug in
either side. The regression test
(\texttt{tests/test\_delta\_sigma\_prj.py}) encodes this as a
per-\(R\) tolerance (\(0.5\%\) on \(R\!\ge\!1\), \(2.5\%\) at
\(R\!=\!0.3\)).

\begin{figure}[!htbp]
\centering
\includegraphics[width=0.92\textwidth]{figs/pedag_delta_sigma_quad.png}
\caption{Precision audit of production \texttt{DeltaSigmaPrj} (filled
markers, solid) against the \texttt{scipy.quad} reference (open
markers) at \((\lob,\zob)\!=\!(20,0.5)\). Top: absolute profiles
for \texttt{total}, \texttt{cl} (default return), and \texttt{rnd}
(truncation-dependent boundary term). Bottom: fractional residual
\((\mathrm{prod}-\mathrm{quad})/\mathrm{quad}\), with
\(\pm0.5\%\) guides (dotted). Agreement is sub-\(0.5\%\) for
\(R\!\ge\!1\,\hMpc\); at \(R\!=\!0.3\) the \(\sim2\%\) residual
tracks a real quadrature-method difference where
\(\theta_{\rm excl}(z)\sim\theta_R\) (see text).}
\label{fig:dsprj_quad}
\end{figure}
...
\paragraph{\texttt{Nz\_bias}: a smaller, decoupled budget for \texttt{\_P\_operator}.}
The \(N_z\!=\!80\) budget above is tuned to \(\avg{\Sprj}\)'s own
needs via \texttt{SigmaPrj.\_build\_zM\_context} (which calls the same
\texttt{\_z\_grid}) and is unchanged. \texttt{SelBias.\_P\_operator}
now reads a separate \texttt{GridConfig.Nz\_bias} (default \(48\)) for
its own \((\Pop[1],I_1,I_2)\) z-grid. The ring+outer split enforces
hard floors (\(N_{\rm ring}\!\geq\!9\), \(N_{\rm outer}\!\geq\!15\) per
side), so any \(N_z\) below \(\sim\!40\) collapses to the same
39-node grid; \(48\) sits just above that floor. Against an
\(N_z\!=\!200,N_\theta\!=\!30\) reference across the 12 DES Y3
\((\lob,\zob)\) bins, \(N_z\!=\!48\) reaches \(0.045\%\) worst-case
error (regression test
\texttt{TestSelBiasNzBias::test\_default\_Nz\_bias\_sub\_01pct\_of\_quad}),
comfortably inside the \(0.1\%\) bar the quad-matched tests enforce,
for a measured \(\sim\!1.6\)--\(1.7\times\) reduction in per-call
evaluation time (\texttt{validations/nz\_bias\_convergence.py}). An
earlier design replaced the ring/outer GL entirely with a
polynomial-fit-plus-closed-form scheme (sampling \(K\!\sim\!5\)--9
nodes and integrating the fit analytically); that was dropped after
finding \(\xinl\) goes negative over \(\sim\!20\)--\(25\%\) of the
outer decay range at higher \(\zob\) (BAO-scale separations), which
breaks a log-space power-law fit. The existing GL scheme has no such
hazard, and — per the floors above — already had slack.
...
Grid-size defaults: \(N_z\!=\!80\) (read by \texttt{SigmaPrj}/
\texttt{DeltaSigmaPrj} via \texttt{SelBias.\_z\_grid}),
\(N_z^{\rm bias}\!=\!48\) (read only by
\texttt{SelBias.\_P\_operator}, \S\ref{sec:zgrid}), \(N_\theta\!=\!10\),
...
\midrule
\(\rhoprj\) & Per-halo projection weight \(w_z\,f_A\,\lambda\) (\(\lambda<\lob\)) &
...
\(\bar{b}_{\rm small},\bar{b}_{\rm large}\) & \(\ltr\)-averaged plateaus (cached per \((\lob,\zob)\)) &
  \eqref{eq:b_bar_plateaus} & \texttt{SelBias.\_marginalised\_plateaus(lob,zob,pre,...)} \\
\(\bsel(\lob,\zob,\theta)\) & \(\ltr\)-marginalised bias; input to \(\avg{\Sprj}\) &
  \eqref{eq:b_marg_sigmoid} & \texttt{SelBias.b\_sel\_marginalised(th,lob,zob,precomp=pre)} \\
\midrule
...
\(\Delta\Smis\) & Miscentered NFW radial excess
  \(\bar\Sigma_{\rm mis}(<\!R)-\Smis(R)\) (companion lookup table) & --- &
  \texttt{NFWMiscentered.delta\_sigma\_grid} \\
...
\(\avg{\Delta\Sigma^{\rm prj}(R\mid\lob,\zob)}\) & Two-halo lensing excess
  (see \texttt{docs/delta\_sigma\_prj\_derivation.tex}) & --- &
  \texttt{DeltaSigmaPrj(cosmo,sel\_bias,nfw)(R,lob,zob)} \\
\bottomrule
\end{tabular}
\end{center}
