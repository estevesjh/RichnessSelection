...
\notag\\
&\times\;
\DSmis\bigl(R\mid M,z,\Rmis=\theta\,\chi(\zob)\bigr),
\label{eq:DSprj}
\end{align}
where $\DSmis$ is the excess surface density of a halo of mass
$M$ miscentred by the transverse offset $\Rmis$ corresponding to
the angular separation $\theta$, and the integral is understood
to be normalised by the same $(z,M)$ halo measure. Every
...
\qquad
\dprjx
= \frac{\lob-\ltr}{\Dprnd} - 1,
\label{eq:bls}
\end{equation}
where $\dprjx$ measures the excess of the observed inflation over
the uniform-interloper expectation $\Dprnd$ of
eq.~\eqref{eq:Dprj_rnd}. Second, the observed inflation must
...
\item form $\dprjx$ and the large-scale plateau $\brmls$,
      eq.~\eqref{eq:bls};
...
\label{eq:alg_split}
\end{equation}
The \emph{exclusion zone}, $|\pi|\le\pis\equiv20\,\Rex$, is the
...
Validated against \texttt{scipy.quad} on
eq.~\eqref{eq:step1}: $0.006$--$0.04\%$ on the assembled
operators, $\le0.07\%$ of the profile scale on the end-to-end
$\brm(\theta)$ --- ss and ls share one pipeline, so their errors
are correlated and largely cancel in the $\brmss$ closure of
eq.~\eqref{eq:bss}. Both far below the $0.1\%$ pipeline
tolerance.
